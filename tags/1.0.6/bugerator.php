...
class BugeratorMain {
	...
	if (-1 == $project and isset($_GET['project'])) {
	    $project = intval($_GET['project']);
	}
	...
	if ("ALL" == $project) {
	    ...
	    $sql = "SELECT name FROM $bugerator_project_table WHERE id = '$project'";
...
function show_file_$type" . "_$id() {
	...
	jQuery.get(ajaxurl, data, function(response) {
	    ...
	    if (true == FORCE_SSL_ADMIN)
		$ajax_url = str_repalce("http:","https:",$ajax_url);
	...
	$total_count = $wpdb->get_var($wpdb->prepare($sql));
	...
	if ($total_count > 5) {
	    // Arrow links:
	    // double left
	    $link_db_left = "onClick='bug_get_list(0,$num_things,$show_hidden)'";
	    // double right (last)
	    $link_db_right = "onClick='bug_get_list(" .( $total_count -1) . ",$num_things,$show_hidden)'";
	    // left one page
	    $left_page = $original_page_start - $num_things;
	    if ($left_page < 0)
		$left_page = 0;
	    $link_left = "onClick='bug_get_list($left_page,$num_things,$show_hidden)'";
	    // right one page
	    $right_page = $original_page_start + $num_things;
	    if ($right_page >= $total_count)
		$right_page = $total_count -1;

	    $link_right = "onClick='bug_get_list($right_page,$num_things,$show_hidden)'";
	    // what page are we on?
	    ...
	    if ($project_id > 0) {
		if (0 == $show_hidden)
		    $output .= "<a onClick='bug_get_list($original_page_start,$num_things,1)'>Show hidden</a><br/> ";
		else
		    $output .= "<a onClick='bug_get_list($original_page_start,$num_things,0)'>Hide hidden</a><br/> ";
	    }
...
heading names (ie. bugerator_css_all) then the program won't be able to parse this. */\r\n";
	...
	$sql = "INSERT INTO $bugerator_project_table ( name, owner, " .
		"current_version, version_date, created_date, status, version_list, version_created_list,
                    version_goal_list, admins, hidden) VALUES ( '$project_name', '$owner->ID', '$version', " .
		"'$goal_date', '$current_date', '" . $status_array[$_POST['project_status']] . "', " .
	...
	if (isset($_POST['default_version']) and $options['default_version'] <> $_POST['default_version']) {
