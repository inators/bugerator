...
class Ajax {
	...
	foreach ($results as $result) {
	    ...
	    if (isset($goals[$next_version_index + 1]))
		$next_date = date($date_format, strtotime($goals[$next_version_index + 1]));
	    else
		$next_date = "";
	    // get admin list
...
class BugeratorMain {
	...
	for ($x = 0; $x < count($results); $x++) {
	    ...
	    $key = array_search("0.0", $version_list);
