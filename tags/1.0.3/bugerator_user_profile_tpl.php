...
<?PHP else: ?>
	...
	<?PHP if ("Projects" == $row->type): ?>
	    <ul class='bugerator bugerator_profile'>
		    ...
		    <?PHP echo $project_names[$row->foreign_id]; ?> 
	    	<a href=<?PHP echo $post->guid; ?>&bugerator_nav=profile&project=<?PHP echo $project_id; ?>&unsubscribe=Projects&id=<?PHP echo $row->id; ?>&nonce=<?PHP echo $nonce; ?> >
	    	   Click to unsubscribe
	        </a>
	<?PHP else: ?>
	    <li class='bugerator bugerator_profile'>
	        <strong>Issue:</strong> <?PHP echo $issue_names[$row->foreign_id] ?>
	        <a href=<?PHP echo $post->guid; ?>&bugerator_nav=profile&project=<?PHP echo $project_id; ?>&unsubscribe=Issues&id=<?PHP echo $row->id; ?>&nonce=<?PHP echo $nonce; ?> >
	           Click to unsubscribe
	    </a>
	       <?PHP endif;
	   endforeach;
